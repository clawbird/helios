    ...
    impl PartialViewDataStorage {
        ...
        pub fn query_and_store(&mut self, address: Address) -> Result<()> {
            ...
            if self.address_queue.len() > self.max_queue_capacity {
                ...
                // https://doc.rust-lang.org/std/collections/struct.VecDeque.html#method.pop_back
                if let Some(removed_address) = self.address_queue.pop_back() {
                    return Ok(());
                }
                return Err(eyre!("unable to remove least recently accessed address"));
            }
            return Err(eyre!("address queue must be greater than max queue capacity"));
        }
    }
}
